...
exports.create = () => {
        ...
        osprey.loadFile(ramlSpecFile)
            .then((middleware) => {
                ...
                appServer.use((err, req, res, next) => {
                    ...
                    httpResponseTemplates.send404(res, "The requested URL was not found. Please try again.");
                    next(err);
                });
     ...
     */
    appServer.installApiDocs = (ramlSpecFile) => {
        const apiSpecURL = appServer.getPath("/specs");
    ...
    appServer.listen = (callback) => {
        http.createServer(appServer).listen(config.serverPort, config.serverURL, () => {
            logger.info(`Server listening on ${config.serverURL}:${config.serverPort}`);
            if (callback) {
                callback(null);
                return;
            }
